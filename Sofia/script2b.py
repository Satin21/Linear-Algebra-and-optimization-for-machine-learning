...
def qr_fact(X):
    """
    Function performing QR factorization of a matrix X via the 
    Gram{Schmidt orthonormalization algorithm as described on page 
    39 of the notes.
    """
    m,n = np.shape(X)
    Q = np.array(X)      
...
def qr_eig(X):
    ...
    39 of the notes.
    """
    Anow = copy.deepcopy(X)
    test_quant = 1
    tol = 1e-3
    idx_iter = 0
    max_iter = 50
    Q_all = np.eye(np.shape(X)[0])
    while idx_iter < max_iter and test_quant > tol:
        # print(Anow)
        Q,R = qr_fact(Anow)
        # Q,R = qr(Anow)  # Built-in function was used for testing
        # print(Q_all)
        Q_all = Q_all @ Q
        Anext = R @ Q
...
eigenValues, eigenVectors = qr_eig(L)
